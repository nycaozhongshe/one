<template>
	<div id="title">
		<m-header :headShow="true">一个阅读</m-header>
		...
		<scroll :data='contentList' @scrollToEnd="refresh" :callbackTop='2'>
			<div class="index-content" id="sss">
				<div class="index-content-list" v-for='(item,index) in contentList'>
					<router-link :to="{path:'/info',query: {id:item.item_id ,type:item.content_type}}" tag='div'>
...
<script>
	...
	export default {
		...
		created() {
			this.$http.get('http://v3.wufazhuce.com:8000/api/channel/reading/more/0?channel=wdj&version=4.0.2&uuid=ffffffff-a90e-706a-63f7-ccf973aae5ee&platform=android')
				.then(
					(data) => {
		...
		methods: {
			...
			refresh() {
				this.onList = 'http://v3.wufazhuce.com:8000/api/channel/reading/more/'
